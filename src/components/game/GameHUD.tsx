...
export default function GameHUD() {
  ...
  return (
    <div className="w-full max-w-[480px] mx-auto glass-panel rounded-2xl p-3 border border-white/10 shadow-xl mb-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-700/60 shadow-inner">
          ...
          <span className="font-mono font-bold text-sm sm:text-base text-white">
            {matchedCount} <span className="text-xs text-slate-400 font-normal">/ {TOTAL_PAIRS}</span>
          </span>
        </div>

        {/* Moves & Combo */}
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-slate-800/80 px-2.5 py-1.5 rounded-xl border border-slate-700/60 shadow-inner text-xs">
            <span className="text-slate-400">Moves:</span>
            <span className="font-mono font-bold text-slate-200">{movesCount}</span>
          </div>

          {comboCount > 1 && (
            <div className="flex items-center gap-1 bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2 py-1 rounded-xl text-xs font-bold animate-bounce shadow-sm">
      ...
      {/* Progress Bar */}
      <div className="w-full h-1 bg-slate-800 rounded-full mt-2.5 overflow-hidden border border-slate-700/40">
        <div
